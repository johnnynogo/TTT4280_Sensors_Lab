...
for i in range(len(filenames)):
    cap = cv2.VideoCapture(filenames[i], cv2.CAP_FFMPEG)
    if not cap.isOpened():
    ...
    np.savetxt(output_filename[i], np.flip(mean_signal, 1))
    print("Data saved to '" + output_filename[i] + "', fps = " + str(fps) + " frames/second")
